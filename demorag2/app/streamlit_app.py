...
def check_collection_exists(client):
    ...
    try:
        ...
        return any(c.name == "products" for c in collections)
    except:
        return False


def get_collection(client):
    """Get products collection with error handling"""
    if not client:
        return None
    
    try:
        return client.get_collection(name="products")
    except Exception as e:
        return None
...
def main():
    ...
    if not collection_exists:
        ...
        st.markdown("""
        The product database hasn't been created yet. Please run the initialization script first:
        ...
        **First run takes 5-10 minutes** (downloads AI models)
        
        After initialization completes, refresh this page.
        """)
        ...
        with st.expander("🔍 Troubleshooting"):
            ...
            **If the command above doesn't work:**
            ...
            4. See **TROUBLESHOOTING.md** for complete solutions
            """)
    ...
    with tab1:
        ...
        if search_button and question:
            with st.spinner("Searching..."):
                ...
                if not collection:
                    st.error("❌ Could not access product collection. Please ensure the demo has been run.")
                    st.code("docker exec -it iceberg-app python /app/scripts/demo.py")
                    return
...
Provide a clear, concise answer focusing on the most relevant products. Include specific details like prices and features."""
    ...
    with tab2:
        ...
        if not collection:
            st.warning("⚠️ Product collection not found. Please run the demo first:")
            st.code("docker exec -it iceberg-app python /app/scripts/demo.py")
        else:
            try:
                # Get all products
                ...
                st.info("💡 Run the demo first: `docker exec -it iceberg-app python /app/scripts/demo.py`")
    ...
    with tab4:
        ...
        if not collection:
            st.warning("⚠️ No data available. Please run the demo first:")
            st.code("docker exec -it iceberg-app python /app/scripts/demo.py")
        else:
            try:
                results = collection.get()
